...
function usage(): never {
  console.error("Usage: npx tsx process/agile/automation/add-to-sprint.ts <ticket> <sprint>");
...
// Move ticket
fs.renameSync(sourcePath, path.join(sprintDir, fileName));
console.log(`${ticketName} → ${sprintName}`);
...
for (const sub of subtasks) {
  ...
  console.log(`${sub.replace(".md", "")} → ${sprintName}`);
}
...
if (fs.existsSync(sprintDocPath)) {
  ...
  const ticketLine = `- ${ticketName}`;
  if (!doc.includes(ticketLine)) {
    doc = doc.replace(/^(## Tickets\n)/m, `$1${ticketLine}\n`);
    fs.writeFileSync(sprintDocPath, doc, "utf-8");
  }
}

console.log(`Added ${ticketName} (${subtasks.length} subtasks) to ${sprintName}.`);
