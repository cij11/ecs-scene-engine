...
function usage(): never {
  ...
  console.error("  sprint add <ticket> <sprint>");
  console.error("  sprint start <sprint>");
  console.error("  sprint complete <sprint>");
  console.error("  sprint return <ticket> <sprint>");
  console.error("  sprint velocity");
  console.error("");
  console.error("Migration:");
  console.error("  migrate run");
  process.exit(1);
}
...
try {
  switch (domain) {
    ...
    case "migrate":
      ...
      break;
    default:
      console.error(`Unknown domain: "${domain}". Use "ticket" or "sprint".`);
      process.exit(1);
  }
} catch (e: unknown) {
  handleError(e);
...
function handleTicket(action: string, args: string[]): void {
  switch (action) {
    ...
    case "status": {
      ...
      const { ticket, oldStatus } = service.transitionTicket(
        name,
        newStatus,
        team,
      );
    ...
    case "validate": {
      ...
      if (errors.length === 0 && warnings.length === 0) {
        console.log(
          `All ${tickets.length} tickets validated. No issues found.`,
        );
      }
      break;
    }
    ...
    case "list": {
      ...
      for (const t of tickets) {
        const sprint = t.sprintName ? ` [${t.sprintName}]` : "";
        console.log(
          `  ${t.name.padEnd(maxNameLen)}  ${t.status.padEnd(20)}  ${t.title}${sprint}`,
        );
      }
      break;
    }
...
function handleSprint(action: string, args: string[]): void {
  switch (action) {
    ...
    case "complete": {
      ...
      console.log(
        `  Tickets: ${sprint.completedTickets}/${sprint.totalTickets} done`,
      );
      console.log(
        `  Points:  ${sprint.completedPoints}/${sprint.totalPoints} completed`,
      );
      console.log(`  Hours:   ${sprint.hours}`);
      console.log(
        `  Velocity: ${sprint.completedPoints} points in ${sprint.hours} hours`,
      );
      break;
    }
